...
def match_val_to_cloud(val_index, val_cph, val_cth, val_cth_std, cloud_lat, cloud_lon,
    ...
    if "tree" in val_index:
        dist_deg, nn = val_index["tree"].query(q, k=1)
    ...
    out_cph = val_cph[ii, jj].astype(np.float64, copy=False)
    out_cth = val_cth[ii, jj].astype(np.float64, copy=False)
    out_std = val_cth_std[ii, jj].astype(np.float64, copy=False)
