...
for MONTH in {01..12}; do
    for part in {01..02}; do
        config_name="${GTE_DIR}/configs/Validation_DARDAR/02_deg/${YEAR}_tracking/${MONTH}_${part}.yaml"
        job_name="${MONTH}_${part}_${YEAR}_postproc"
        postproc_job_id=$(sbatch --begin=now+300minutes --parsable -J "$job_name" "${GTE_DIR}slurm_jobs/3_postprocessing/postproc_job.bsub" -c $config_name)
        postproc_job_ids+=( "$postproc_job_id" )
    done
done

glac_name="${YEAR}_glac"
GTE_CONFIG_DIR="${GTE_DIR}/cluster/work/climate/dnikolo/n2o/Glaciation_time_estimator/configs/Validation_DARDAR/02_deg/${YEAR}_tracking/01_01.yaml"
