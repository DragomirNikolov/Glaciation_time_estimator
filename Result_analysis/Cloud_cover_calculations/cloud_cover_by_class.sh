...
if [ -z "$MONTH" ]; then
    echo "Month variable is empty. Exiting..."
    exit 1
fi
...
for pole in sp np
do
    month_dir="${CLAAS_DIR}${pole}/${year}/${month}/"
    ...
    do
        ...
        cdo merge "${CLAAS_DIR}/${pole}/CM_SAF_CLAAS3_L2_AUX.nc" "combined_total.nc" combined_aux.nc
        ...
        if [[ $day_dir =~ ([0-9]{4})/([0-9]{2})//([0-9]{2})/ ]]; then
            ...
            echo "${CLAAS_DIR}/Cloud_cover_by_class/${pole}/$filename"
            cdo timmean class_cover.nc "${CLAAS_DIR}/Cloud_cover_by_class/${pole}/$filename"
        else
